...
def measure_keyword_legacy(binary: Path, runs: int, warmup: int, file_count: int) -> float:
    def worker() -> float:
...
def measure_keyword_ranking(binary: Path, runs: int, warmup: int, file_count: int) -> float:
    def worker() -> float:
...
def measure_identifier_ranking(binary: Path, runs: int, warmup: int, file_count: int) -> float:
    def worker() -> float:
...
def measure_scoped_ranking(binary: Path, runs: int, warmup: int, file_count: int) -> float:
    def worker() -> float:
...
def measure(binary: Path, runs: int, warmup: int, file_count: int) -> dict[str, float]:
    ...
    keyword_legacy_ms = measure_keyword_legacy(binary, runs, warmup, file_count)
    keyword_ranking_ms = measure_keyword_ranking(binary, runs, warmup, file_count)
    identifier_ranking_ms = measure_identifier_ranking(binary, runs, warmup, file_count)
    scoped_ranking_ms = measure_scoped_ranking(binary, runs, warmup, file_count)

    return {
        "cold_index_ms": round(cold_index_ms, 2),
        "cold_index_throughput_fps": round(file_count / max(cold_index_ms / 1000.0, 0.001), 2),
        "keyword_legacy_ms": round(keyword_legacy_ms, 2),
        "keyword_ranking_ms": round(keyword_ranking_ms, 2),
        "identifier_ranking_ms": round(identifier_ranking_ms, 2),
        "scoped_ranking_ms": round(scoped_ranking_ms, 2),
    }
