...
v1.4.7 핵심:
- 사용자/코딩 에이전트 모두를 위해 search/MCP UX를 더 엄격하고 예측 가능하게 정리했습니다.
- MCP 신뢰성 수정으로 `-` 시작 리터럴 쿼리, `cwd` 기준 경로 해석, search 결과 경로 round-trip 안정성을 강화했습니다.
- 최신 Codex PyTorch 재측정 결과에 맞춰 문서와 벤치마크 스냅샷을 동기화했습니다.
