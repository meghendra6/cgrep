...
인덱싱이 도는/멈추는 시점:
- 일반 CLI/MCP 검색 계열: 호출 시점 auto-index 점검 기반(상주 루프 없음).
- daemon 모드: daemon이 실행 중일 때만 이벤트 기반으로 동작.
- 새 파일 이벤트도 없고 도구 호출도 없으면 추가 재인덱싱 없음.
- `cgrep daemon stop` 실행 시 daemon 기반 인덱싱은 즉시 중단됩니다.
...
복잡한 튜닝 전에 아래 기준값부터 적용하세요.

- 우선 적응형 모드는 유지 (`--no-adaptive`는 고정 타이밍이 필요할 때만).
- 시작 권장값:
  - `--min-interval 180`
  - `--debounce 30`
  - `--max-batch-delay 180` ~ `240`
- 브랜치 전환 시 이벤트 폭주가 잦다면 daemon을 유지한 채 bulk 자동 전환에 맡기는 편이 안정적입니다.
- 워크플로우상 인덱싱 비용을 더 줄여야 하면, 기본은 `cgrep index --background`(1회성)로 운영하고 활성 코딩 구간에서만 daemon을 켜세요.
